...
class ShowUsersTable:
    ...
    def addUser_window(self):
        ...
        self.lbl_addError = tk.Label(self.addUser_window, text="", fg="red", font="Sans 15")
        self.lbl_addError.grid(row=0, columnspan=3)

        lbl_email = tk.Label(self.addUser_window, text="E-mail", font="Sans 13", padx=5, pady=5)
        lbl_password = tk.Label(self.addUser_window, text="Password", font="Sans 13", padx=5, pady=5)
        ...
        self.btn_check = tk.Checkbutton(self.addUser_window, text='Get full access', variable=self.check_value,
    ...
    def deleteUser_window(self):
        ...
        if self.id_select:
            ...
            lbl_confirm = tk.Label(self.deleteUser_window, text="Are you sure?", font="Sans 21")
            lbl_confirm.pack()

            btn_cancel = tk.Button(self.deleteUser_window, text="No", width=7, command=self.deleteUser_window.destroy)
            btn_confirm = tk.Button(self.deleteUser_window, text="Yes", width=7, command=self.deleteUser_row)
    ...
    def error_window(self):
        ...
        label = tk.Label(error_window, text="Something went wrong. Fill in only one field")
        label.pack()
        button = tk.Button(error_window, text="Ok", width=10, command=error_window.destroy)
    ...
    def sortUser_window(self):
        ...
        lbl_title = tk.Label(self.sortUser_window, text="Choose column you want to sort", font="Sans 15")
        lbl_title.pack()

        btn_idColumn = tk.Button(self.sortUser_window, text="ID", command=lambda: self.sort_column("id"))
        btn_emailColumn = tk.Button(self.sortUser_window, text="E-mail", command=lambda: self.sort_column("email"))
        btn_passwordColumn = tk.Button(self.sortUser_window, text="Password", command=lambda: self.sort_column("password"))
        btn_permissionColumn = tk.Button(self.sortUser_window, text="Permission", command=lambda: self.sort_column("permission"))
    ...
    def findUser_window(self):
        ...
        lbl_combobox_title = tk.Label(self.findUser_window, text="Choose column you want to find", font="Sans 21")
        self.lbl_findError = tk.Label(self.findUser_window, text="", font="Sans 15", fg="red")
        lbl_combobox_title.pack()
        self.lbl_findError.pack()

        self.columns = ["email", "password", "permission"]
        self.selected_column = tk.StringVar()
        self.cmb_find = ttk.Combobox(self.findUser_window, textvariable=self.selected_column, width=12)
        ...
        btn_search = tk.Button(self.findUser_window, text="Search", command=self.search_rows)
    ...
    def search_rows(self):
        if self.choice_find != None:
            ...
            response = RootApp.Root.app.search_element("users", self.choice_find, self.ent_search.get())
            if self.isValid and response:
                self.findUser_window.destroy()
                self.showUser_window.destroy()

                refresh_searchRows_window = tk.Tk()
                refresh_searchRows = ShowUsersTable(refresh_searchRows_window, isFind=True, findData=response)
    ...
    def validate_column(self, choice, command="search"):
        if choice == "email":
            self.validate_add(email=self.ent_search.get() if command == "search" else self.ent_update.get())
            self.isValid = self.valid_email
        if choice == "password":
            self.validate_add(password=self.ent_search.get() if command == "search" else self.ent_update.get())
            self.isValid = self.valid_password
        if choice == "permission":
            self.validate_add(permission=self.ent_search.get() if command == "search" else self.ent_update.get())
    ...
    def updateUser_window(self):
        if self.id_select != 0:
            ...
            lbl_updateUser = tk.Label(self.updateUser_window, text="Choose field for updating.", font="Sans 21")
            self.lbl_error_updateUser = tk.Label(self.updateUser_window, text="", fg="red", font="Sans 16")
            lbl_updateUser.pack()
            self.lbl_error_updateUser.pack()

            self.update_columns = ["email", "password", "permission"]
            self.update_selected_column = tk.StringVar()
            self.cmb_update = ttk.Combobox(self.updateUser_window, textvariable=self.update_selected_column, width=12)
            ...
            btn_update = tk.Button(self.updateUser_window, text="Update", command=self.update_user)
            btn_update.pack(side=tk.LEFT)
    ...
    def update_user(self):
        if self.choice_update != None:
            ...
            if self.isValid:
                RootApp.Root.app.update_element("users", self.choice_update, self.ent_update.get(), self.id_select)
